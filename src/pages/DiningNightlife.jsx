...
function TamarindHavenPortfolio() {
  ...
  const heroImages = ['/lalibela.jpg', '/deplom p.jpg', '/shebele.jpg'];
  ...
  const data = {
    events: [
      ...
      { name: "Abay Hall", cap: "300 Pax", img: "/abayh.jpg", desc: "Sophisticated corporate conference venue." },
      ...
      { name: "Qouqa Business Hall", cap: "150 Pax", img: "/Qokah.png", desc: "High-speed business negotiation hub." },
  ...
  return (
    <div className="bg-[#FBFBFA] text-[#2B7A4B] font-sans min-h-screen">
      ...
      <div className="flex justify-center gap-8 py-10 bg-white shadow-sm">
        {['events', 'parking'].map(tab => (
          <button 
            ...
            className={`text-[11px] font-black uppercase tracking-[0.3em] pb-1 border-b-2 transition-all duration-300 
              ${activeTab === tab 
                ? 'text-[#2B7A4B] border-[#2B7A4B] font-black' 
                : 'text-[#2B7A4B] border-transparent hover:border-[#2B7A4B] font-bold'
              }`}
          >
            {tab}
          </button>
      ...
      {/* DYNAMIC GRID - UPDATED TITLES TO CENTERED AND BOLD */}
<main className="max-w-7xl mx-auto px-4 py-16">
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    {data[activeTab].map((item, i) => (
      <div key={i} className="bg-white border p-6 flex flex-col items-center text-center hover:border-[#2B7A4B] transition-all shadow-sm">
        <img src={item.img} alt={item.name} className="w-full h-48 object-cover mb-6" />
        
        {/* Title updated: added 'font-bold' and 'text-center' */}
        <h3 className="text-sm font-black uppercase tracking-widest mb-1 text-[#2B7A4B] text-center font-bold">
          {item.name}
        </h3>
